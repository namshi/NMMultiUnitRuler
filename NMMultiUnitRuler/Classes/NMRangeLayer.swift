...
public enum NMMarkerVerticalAlignment: Int {
    case bottom = 0, center, top
}


public class NMRange<T>: NSObject {
    open var location: T
    open var length: T
...
public class NMRangeMarkerType: NSObject, NSCopying {
    ...
    class func minScale(types: Array<NMRangeMarkerType>?) -> Float {
    ...
    class func largestScale(types: Array<NMRangeMarkerType>?) -> Float {
...
class NMRangeMarker: NSObject {
    var type: NMRangeMarkerType = NMRangeMarkerType()
    var value: Float = 0.0
    var alignment: NMMarkerVerticalAlignment = NMMarkerVerticalAlignment.top
    var text: String = ""
    var textAlignment: NMMarkerVerticalAlignment = NMMarkerVerticalAlignment.bottom
...
class NMRangeLayer: CALayer {
    var direction: NMLayerDirection = .horizontal
    var markerTypes: Array<NMRangeMarkerType> = Array()
    var colorOverrides: Dictionary<NMRange<Float>, UIColor>?

    var range: NMRange<Float> = NMRange<Float>(location: 0, length: 0) {
        didSet {
            if range.length != 0 {
    ...
    lazy var markers: Array<NMRangeMarker> = self.initializeMarkers()
    ...
    func initializeMarkers() -> Array<NMRangeMarker> {
        var valueToMarkerMap: [Float: NMRangeMarker] = [:]
    ...
    */
    func drawLayer() {
        ...
        var previousMarker: NMRangeMarker?
    ...
    /*
        ...
        marker
    */
    func drawMarkerInHorizontalDirection(_ marker: NMRangeMarker, at pos: CGFloat, in context: CGContext) {
        let rangeEnd = fmax(self.range.location, self.range.location + self.range.length)
    ...
    func drawMarkerInVerticalDirection(_ marker: NMRangeMarker, at pos: CGFloat, in context: CGContext) {
        let rangeEnd = fmax(self.range.location, self.range.location + self.range.length)
        let colorOverride = self.colorOverride(for: marker.value)
        let color = colorOverride ?? marker.type.color
        let textAttributes = [
