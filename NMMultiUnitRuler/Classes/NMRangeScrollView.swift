...
class NMRangeScrollView: UIControl, UIScrollViewDelegate {

    open var sideOffset: CGFloat = kDefaultScrollViewSideOffset
    open var direction: NMLayerDirection = .horizontal
    open var colorOverrides: Dictionary<NMRange<Float>, UIColor>?
    open var range: NMRange<Float> = NMRange<Float>(location: 0, length: 0) {
        didSet {
            setupScrollView()
            currentValue = ceilf((range.location + range.length) / 2.0)
        }
    }
    var markerTypes: Array<NMRangeMarkerType>? {
        didSet {
            setupScrollView()
        }
    }
    ...
    var rangeLayer: NMRangeLayer = NMRangeLayer()
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if (!automaticallyUpdatingScroll) {
            ...
            let minScale = NMRangeMarkerType.minScale(types: self.markerTypes)
    ...
    */
    ...
    func frameForRangeLayer() -> CGRect {
        let maxScale = NMRangeMarkerType.largestScale(types: self.markerTypes)
